...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, prob in self.mdp.getTransitionStatesAndProbs(state, action):
            Qvalue = Qvalue + prob * (self.mdp.getReward(state, action, nextState) + self.discount * self.values[nextState])
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            # update one state
            k = i % len(self.mdp.getStates())
            state = self.mdp.getStates()[k]
    ...
    def runValueIteration(self):
        ...
        for s in self.mdp.getStates():
            ...
            if not self.mdp.isTerminal(s):
                ...
                max_value = max([self.computeQValueFromValues(s, action) for action in self.mdp.getPossibleActions(s)])
                # print(max_value)
                diff = abs(self.values[s] - max_value)
                pq.update(s, -diff)
        ...
        for i in range(self.iterations):
            ...
            for p in predecessors[s]:
                if not self.mdp.isTerminal(p):
                    ...
                    p_max_value = max([self.computeQValueFromValues(p, action) for action in self.mdp.getPossibleActions(p)])

                    p_diff = abs(self.values[p] - p_max_value)
